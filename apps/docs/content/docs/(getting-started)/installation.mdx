...
async function handleRequest(request: Request) {
  return tailorKit.handler(request, {
    ...
    authenticate: async () => {
      // [!code highlight]
      const session = await auth(request); // [!code highlight]
      ...
      return {
        // [!code highlight]
        actionContext: { user: session.user }, // [!code highlight]
        // Used to separate installed apps for each user, team, or project. // [!code highlight]
        scopeId: session.user.id, // [!code highlight]
      }; // [!code highlight]
    }, // [!code highlight]
  }); // [!code highlight]
}
...
app.on(["GET", "POST"], "/api/tailorkit/*", async (c) => {
  return tailorKit.handler(c.req.raw, {
    ...
    authenticate: async () => {
      // [!code highlight]
      const session = await auth(c.req.raw); // [!code highlight]
