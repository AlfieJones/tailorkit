...
component name, its serializable fields, its callbacks, and its children.
...
- `children: true`: allows nested text and components. Omit it for leaf components.
...
Fields use the same schema model as views and actions. The example uses Zod
...
Use [Actions](/docs/integrate/actions) for trusted server-side work such as database
writes, permission checks, and external API calls.

## Children

Set `children: true` to allow nested text and components. Children preserve their order and can contain multiple elements.
...
On the host, the React renderer receives children as React nodes.

```tsx
TabsList: ({ props, children }) => <TabsList {...props}>{children}</TabsList>,
```

A component without `children: true` is a
...
renderer props and children are typed from your component definitions.
...
<Callout title="Keep Components Stable">
  ...
  support for children can break existing apps. Add new optional fields, new callback payload
  fields, or new components when possible. When you need a breaking change, version the component
  name or coordinate the app update.
</Callout>
...
events, children for child composition, and actions for trusted server-side work.
